...
/// boost::apply_visitor(GenericVisitorFB<Class1, Class2>(
///     [](Class1& _c) { _c.f(); },
///     [](Class2& _c) { _c.g(); }
/// ), variant);
...
template <class...>
struct GenericVisitorFB{};

template <class Visitable, class... Others>
struct GenericVisitorFB<Visitable, Others...>: public GenericVisitorFB<Others...>
{
	explicit GenericVisitorFB(
		std::function<void(Visitable&)> _visitor,
		std::function<void(Others&)>... _otherVisitors
	):
		GenericVisitorFB<Others...>(_otherVisitors...),
		m_visitor(_visitor)
	{}

	using GenericVisitorFB<Others...>::operator ();
	void operator()(Visitable& _v) const { m_visitor(_v); }

	std::function<void(Visitable&)> m_visitor;
};
template <>
struct GenericVisitorFB<>: public boost::static_visitor<> {
	template <class T>
	void operator()(T&) const { }
};

/// Generic visitor with fallback that can return a value:
/// boost::apply_visitor(GenericVisitorFBReturns<ReturnType, Class1, Class2>(
...
struct GenericVisitorFBReturns{};

template <class R, class Visitable, class... Others>
struct GenericVisitorFBReturns<R, Visitable, Others...>: public GenericVisitorFBReturns<R, Others...>
{
	explicit GenericVisitorFBReturns(
		std::function<R(Visitable&)> _visitor,
		std::function<R(Others&)>... _otherVisitors
	):
		GenericVisitorFBReturns<R, Others...>(_otherVisitors...),
		m_visitor(_visitor)
	{}

	using GenericVisitorFBReturns<R, Others...>::operator ();
	R operator()(Visitable& _v) const { return m_visitor(_v); }

	std::function<R(Visitable&)> m_visitor;
};
template <class R>
struct GenericVisitorFBReturns<R>: public boost::static_visitor<R> {
	template <class T>
